...
# IDEA: Simulate a real BCI with feedback from some ERP decoder - say a wheelchair. Get a seemingly more
#       accurate representation of how our decoder would work in a realistic setting BCI interface
#       with feedback. Basically a thought experiment
#       or  simulation to see if we can find anything interesting
# Process: Train on Offline session 1: This is input data essentially
#           1. All data preprocessing - with CAR, no grid search (for simplicity)
#           2. Train on offline session -> build decoder
#           3. Simulate trials on online session 1
#           4. We expect BCI classification to occur then ERP would show.
#           5. Say a ERP accuracy of 85%
#               - Correctly classified BCI -> No ERP -> Train (ground truth = classification) correct ground truth
#               - Correctly classified BCI -> ERP -> Train (ground truth == opposite of classification) incorrect ground truth
#               - Incorrect classified BCI -> No ERP -> Train (ground truth = classification) Incorrect ground truth
#               - Incorrect classified BCI -> ERP -> Train (ground truth == opposite of classification) Correct ground truth
...
def build_single_training_data(trial, label, win, lap, fs, t_filt, sp_filt, flim, mask):
    win = math.floor(win * fs)  # seconds -> samples
    step = win - math.floor(lap * fs)  # seconds -> samples
    x = np.zeros(np.sum(mask))
    y = []
    ...
    while winflag:  # loop through windows
        ...
        if accum_prob[-1] > thresh[g_truth - 1]:  # compare to correct class threshold
            return {"BCI_probs": BCI_prob, "accum_probs": accum_prob, "decision": g_truth, "correct": 1,
                    "samples_total": samples_total, "samples_correct": samples_correct, "ground_truth": g_truth}
...
for tr, g_truth in zip(online_trials, online_truths):
    ...
    sample_data = [decision["ground_truth"], decision["samples_total"], decision["samples_correct"][0],
                   decision["samples_correct"][1]]
    ...
    # ERP Simulation for retraining - if classification present
    if math.isnan(decision["decision"]):
        # no ERP can be retrained - skip
        pass
    else:
        print("Performing ERP Retrain")
        correct_ground_truth = decision["ground_truth"]
        BCI_classification = decision["decision"]
        random_number = random.random()
        # If BCI Classification Correct
        if decision["correct"] == 1:
            # If random_value < ERP_Accuracy -> train with classification
            if random_number < ERP_Accuracy:
                # Train with BCI Classification
                x1, y1 = build_single_training_data(tr, BCI_classification, win, lap, fs, broad_filt, car_filt, broad, mask)
            # If random_value > ERP_Accuracy -> train with flipped classification
            else:
                BCI_classification = (BCI_classification % 2) + 1
                x1, y1 = build_single_training_data(tr, BCI_classification, win, lap, fs, broad_filt, car_filt, broad, mask)
        # If BCI Not correct
        else:
            # If random_value < ERP_Accuracy -> train !classification
            if random_number < ERP_Accuracy:
                BCI_classification = (BCI_classification % 2) + 1
                x1, y1 = build_single_training_data(tr, BCI_classification, win, lap, fs, broad_filt, car_filt, broad, mask)
            # If random_value > ERP_Accuracy -> train classification
...
sample_data = [subject, electrode, session_id, sample_class_1_accuracy, sample_class_2_accuracy, sample_total_accuracy,
               sample_bias]
